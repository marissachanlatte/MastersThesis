...
%
 ...
 Substituting Eqn.~\eqref{eq:fick_corr} into Eqn.~\eqref{eq:zeroth_moment}, we have the following NDA equation
  ...
  \end{equation}
  $\epsilon_\rg^{k + 1/2}$ can be further broken into the spatial component $\phi_{\epsilon}$ and scattering spectrum $\xi_\rg$ as defined in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{morel-upscat, evans-upscat}}\hspace{0pt}%DIFAUXCMD
.
...
.
  ...
  \DIFdelbegin %DIFDELCMD < \label{eq:decomp}
%DIFDELCMD <   %%%
\DIFdelend \end{equation}
  ...
  By solving Eqn.~\eqref{eq:collapsed} with the same spatial discretization as the NDA solve, we correct the scalar flux from the previous Gauss Seidel iteration as $\phi_\rg^{k+1} = \phi_\rg^{k+1/2} + \epsilon_\rg^{k+1/2}$. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD <   %%%
\DIFdel{TG-NDA Update Procedure
  }%DIFDELCMD < \begin{enumerate}
\begin{enumerate}%DIFAUXCMD
%DIFDELCMD <       \item %%%
\item%DIFAUXCMD
\DIFdel{Solve }%DIFDELCMD < \eqref{eq:k1/2} %%%
\DIFdel{using NDA iteration to find $\phi_\rg^{k+1/2}$ for each group, $\rg$. 
      }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Solve }%DIFDELCMD < \eqref{eq:collapsed} %%%
\DIFdel{to find $\phi_\epsilon^{k+1/2}$. 
      }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Solve }%DIFDELCMD < \eqref{eq:eigenvector} %%%
\DIFdel{to find $\xi$. 
      }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Use }%DIFDELCMD < \eqref{eq:decomp} %%%
\DIFdel{to find $\epsilon_\rg^{k+1/2}$ and apply to each group to get $\phi_\rg^{k+1}$ for all $\rg$. 
      }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Check for convergence between $\phi_\rg^{k+1}$ and $\phi_\rg^{k}$.
      }%DIFDELCMD < \item %%%
\item%DIFAUXCMD
\DIFdel{Repeat until all groups have converged.
  }
