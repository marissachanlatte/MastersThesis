...
NDA adds an extra step to this process by first calculating the angular flux using a higher order equation and then solving the lower order equation using the result of the higher order equation to compute a correction. The result of the lower order solve is then used as the guess on the right hand side during the next iteration.  \DIFdelbegin %DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdel{For ease of reference, we repeat the NDA+SAAF algorithm given in Ch. \ref{sec:intro} using }\DIFdelend \DIFaddbegin \DIFadd{This process is described with }\DIFaddend iteration index $l$ \DIFdelbegin \DIFdel{: 
}%DIFDELCMD < 
