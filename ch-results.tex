...
Importantly, the acceleration in convergence came at no significant cost to accuracy. Figure~\ref{fig:OneMatErr} shows the relative difference per group between the TG-NDA and NDA solutions at each node. The difference is around or below the error tolerance set in the Gauss-Seidel solver as 1e-5, meaning that the solutions agree up to at least four digits, often five. There is no difference between the two solutions in the fast groups as there is no upscattering present in those groups, so the Gauss-Seidel correction does not apply. There is a larger difference in groups 6 and 7, as they have a stronger dependence on the flux of the other groups, so the difference accumulates. Keep in mind that the plot is scaled by 1e-5. 

\begin{figure}[H]
    \centering
    \includegraphics[width=.75\textwidth]{fig/OneMatErr.pdf}
    \caption{Relative Difference Between NDA and TG-NDA A - One Material}
    \label{fig:OneMatErr}
\end{figure}
